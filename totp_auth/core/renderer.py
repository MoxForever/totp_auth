...
from totp_auth.core.database import Server
...
class PageRenderer:
    ...
    def render_for_server(
        ...
        current_widget_name = current_widget_name or server.widgets[0]
        selected_widget = widgets_dict[current_widget_name]
        selected_widget_html = selected_widget.render(
            language.get_lang_for_widget(selected_widget.name)
        )
        del widgets_dict[current_widget_name]
